...
def wbalance_10p(img):
    dr = 1.0 / np.mean(
        np.sort(img[:, :, 0], axis=None)[int(round(-1 * np.size(img[:, :, 0]) * 0.1)) :]
    )
    dg = 1.0 / np.mean(
...
def run_pipeline(img, depths, args):
    ...
    ptsR, ptsG, ptsB = find_backscatter_estimation_points(
        img, depths, fraction=0.01, min_depth_percent=args.min_depth
    )
    ...
    if args.output_graphs:
        ...
        def eval_xs(xs, coefs):
            if len(coefs) == 2:
                ...
                return (coefs[0] * (1 - np.exp(-coefs[1] * xs))) + (
                    coefs[2] * np.exp(-coefs[3] * xs)
                )
    ...
    illR = estimate_illumination(
        img[:, :, 0], Br, nmap, n, p=args.p, max_iters=100, tol=1e-5, f=args.f
    )
    illG = estimate_illumination(
        img[:, :, 1], Bg, nmap, n, p=args.p, max_iters=100, tol=1e-5, f=args.f
    )
    illB = estimate_illumination(
        img[:, :, 2], Bb, nmap, n, p=args.p, max_iters=100, tol=1e-5, f=args.f
    )
    ...
    refined_beta_D_r, coefsR = refine_wideband_attentuation(
        depths, illR, beta_D_r, radius_fraction=args.spread_data_fraction, l=args.l
    )
    beta_D_g, _ = estimate_wideband_attentuation(depths, illG)
    refined_beta_D_g, coefsG = refine_wideband_attentuation(
        depths, illG, beta_D_g, radius_fraction=args.spread_data_fraction, l=args.l
    )
    beta_D_b, _ = estimate_wideband_attentuation(depths, illB)
    refined_beta_D_b, coefsB = refine_wideband_attentuation(
        depths, illB, beta_D_b, radius_fraction=args.spread_data_fraction, l=args.l
    )
    ...
    if args.output_graphs:
        ...
        plt.imshow(
            np.stack(
                [
                    scale(refined_beta_D_r),
                    np.zeros_like(beta_D_r),
                    np.zeros_like(beta_D_r),
                ],
                axis=2,
            )
    ...
    if args.output_graphs:
        ...
        def eval_xs(xs, coefs):
            if len(coefs) == 2:
                ...
                return (coefs[0] * np.exp(coefs[1] * xs)) + (
                    coefs[2] * np.exp(coefs[3] * xs)
                )
        ...
        locs = np.where(
            np.logical_and(
                beta_D_r > eps,
                np.logical_and(
                    beta_D_g > eps, np.logical_and(depths > eps, beta_D_b > eps)
                ),
            )
        )
        plt.scatter(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--f", type=float, default=2.0, help="f value (controls brightness)"
    )
    parser.add_argument(
        "--l",
        type=float,
        default=0.5,
        help="l value (controls balance of attenuation constants)",
    )
    parser.add_argument(
        "--p",
        type=float,
        default=0.01,
        help="p value (controls locality of illuminant map)",
    )
    parser.add_argument(
        "--min-depth",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Preprocess for monodepth depth maps",
    )
    parser.add_argument(
        "--monodepth", action="store_true", help="Preprocess for monodepth"
    )
    parser.add_argument(
        "--monodepth-add-depth",
        type=float,
        default=2.0,
        help="Additive value for monodepth map",
    )
    parser.add_argument(
        "--monodepth-multiply-depth",
        type=float,
        default=10.0,
    ...
    parser.add_argument(
        "--equalize-image",
        action="store_true",
        help="Histogram equalization for final output",
    )
    ...
    if args.preprocess_for_monodepth:
        ...
        if args.monodepth:
            depths = preprocess_monodepth_depth_map(
                depths, args.monodepth_add_depth, args.monodepth_multiply_depth
            )
        else:
        ...
        if args.equalize_image:
            recovered = exposure.equalize_adapthist(
                np.array(recovered), clip_limit=0.03
            )
            sigma_est = estimate_sigma(
                recovered, multichannel=True, average_sigmas=True
            )
